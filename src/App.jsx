...
function useNotifications() {
  const notify = (title, message, icon = "🚕") => {
    ...
    if (Notification.permission === "granted") {
      ...
      Notification.requestPermission().then(p => {
        if (p === "granted") {
          new Notification(`${icon} ${title}`, { body: message, dir: "rtl" });
        }
      });
    }
  };
  ...
  return {
    notifyDriverAccepted: (name, price) => notify("تم قبول طلبك! 🎉", `${name} قبل رحلتك بـ ${price} دج`),
    notifyDriverArriving: (name) => notify("السائق في الطريق 🚕", `${name} سيصل خلال 3 دقائق`),
    notifyRideStarted: () => notify("بدأت رحلتك 🛣️", "استمتع برحلتك — وصولاً آمناً!"),
    notifyRideCompleted: (price) => notify("وصلت بسلام! 🏁", `تم الدفع ${price} دج — شكراً لاستخدامك TaxiDZ`),
    notifyNewOffer: (price) => notify("عرض جديد من سائق 🤝", `سائق قريب يعرض ${price} دج`),
  };
}

// ===== GOOGLE MAP =====
function TaxiMap({ origin, destination, showDrivers, driverPosition }) {
  ...
  const makeIcon = (emoji, bg) =>
    ...
    encodeURIComponent(`<svg xmlns='http://www.w3.org/2000/svg' width='36' height='36' viewBox='0 0 36 36'><circle cx='18' cy='18' r='18' fill='${bg}'/><text x='18' y='25' text-anchor='middle' font-size='18'>${emoji}</text></svg>`);
  ...
  return (
    ...
    >
      ...
      {origin && !directions && <Marker position={origin} />}
      {destination && !directions && <Marker position={destination} />}
      {directions && (
        <DirectionsRenderer directions={directions} options={{ polylineOptions: { strokeColor: C.green, strokeWeight: 4 } }} />
      )}
      {showDrivers && DRIVERS.map(d => (
        <Marker key={d.id} position={d.position} icon={{ url: makeIcon("🚕", C.dark), scaledSize: new window.google.maps.Size(36, 36) }} />
      ))}
      {driverPosition && (
        <Marker position={driverPosition} icon={{ url: makeIcon("🚕", C.orange), scaledSize: new window.google.maps.Size(44, 44) }} />
      )}
    </GoogleMap>
  );
}
      ...
      <div style={{ padding: "16px 20px 12px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          <div style={{ textAlign: "right" }}>
            ...
            <div style={{ fontSize: 13, fontWeight: 700, color: C.text }}>{user?.displayName || user?.email?.split("@")[0] || "مستخدم"}</div>
          </div>
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, fontFamily: "'Cairo', sans-serif", direction: "rtl", paddingBottom: 30 }}>
      ...
      <div style={{ margin: "14px 20px", background: C.card, borderRadius: 24, padding: 20, boxShadow: C.shadow }}>
        ...
        <button onClick={() => originPlace && destPlace && onNext({ originPlace, destPlace, originText, destText, rideType })}
          style={{ width: "100%", background: originPlace && destPlace ? `linear-gradient(135deg, ${C.green}, ${C.greenDark})` : C.border, border: "none", borderRadius: 16, padding: "16px", color: originPlace && destPlace ? "#fff" : C.textMuted, fontFamily: "inherit", fontWeight: 800, fontSize: 16, cursor: originPlace && destPlace ? "pointer" : "default", transition: "all 0.3s" }}>
...
function SearchingScreen({ booking, onDriverFound }) {
  ...
  const { notifyNewOffer, notifyDriverAccepted } = useNotifications();
  ...
  useEffect(() => {
    ...
    if (timer === 4) {
      const price = booking.negotiated ? Math.round(booking.price * 1.1) : booking.price;
      setDrivers(p => p.map((d, i) => i === 0 ? { ...d, status: booking.negotiated ? "counter" : "accepted", offerPrice: price } : d));
      notifyNewOffer(price);
    }
    if (timer === 7) {
      setDrivers(p => p.map((d, i) => i === 1 ? { ...d, status: "accepted", offerPrice: booking.price } : d));
      notifyDriverAccepted("يوسف مزياني", booking.price);
    }
    if (timer === 10) {
      const price = booking.negotiated ? Math.round(booking.price * 1.05) : booking.price;
      setDrivers(p => p.map((d, i) => i === 2 ? { ...d, status: booking.negotiated ? "counter" : "accepted", offerPrice: price } : d));
    }
  }, [timer]);
...
function DriverFoundScreen({ driver, booking, onStart, onCancel }) {
  const { notifyDriverArriving } = useNotifications();

  useEffect(() => {
    notifyDriverArriving(driver.name);
  }, []);
...
function RideScreen({ driver, booking, onEnd }) {
  ...
  const { notifyRideCompleted } = useNotifications();
  ...
  const handleEnd = () => {
    notifyRideCompleted(driver.offerPrice);
    setDone(true);
  };
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, fontFamily: "'Cairo', sans-serif", direction: "rtl" }}>
      ...
      <div style={{ margin: "14px 20px", background: C.card, borderRadius: 24, padding: 20, boxShadow: C.shadow }}>
        ...
        <button onClick={handleEnd} style={{ width: "100%", background: `linear-gradient(135deg, ${C.green}, ${C.greenDark})`, border: "none", borderRadius: 14, padding: 16, color: "#fff", fontFamily: "inherit", fontWeight: 800, fontSize: 16, cursor: "pointer" }}>
          🏁 محاكاة الوصول
        </button>
      </div>
    </div>
...
export default function TaxiDZ() {
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, user => {
      if (user) { setCurrentUser(user); setAuthScreen("app"); }
    });
    return () => unsub();
  }, []);
  ...
  if (!isLoaded) return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Cairo', sans-serif", background: "#f7f3ee" }}>
      <div style={{ textAlign: "center" }}>
        ...
        <div style={{ fontWeight: 700, color: "#1a1a2e" }}>جارٍ تحميل التطبيق...</div>
      </div>
    </div>
  );

  if (authScreen === "welcome") return <div style={{ maxWidth: 390, margin: "0 auto" }}><WelcomeScreen onSelectRole={role => { setSelectedRole(role); setAuthScreen("form"); }} /></div>;
  if (authScreen === "form") return <div style={{ maxWidth: 390, margin: "0 auto" }}><AuthForm role={selectedRole} onSuccess={() => setAuthScreen("app")} onBack={() => setAuthScreen("welcome")} /></div>;
  ...
  return (
    <div style={{ maxWidth: 390, margin: "0 auto", minHeight: "100vh", background: "#f7f3ee" }}>
      ...
      {screen === SCREENS.HOME && <HomeScreen onBook={() => setScreen(SCREENS.BOOKING)} onLogout={handleLogout} user={currentUser} />}
