...
import { WelcomeScreen, AuthForm } from "./Auth";
...
function HomeScreen({ onBook, onLogout, user }) {
  return (
    <div style={{ minHeight: "100vh", background: C.bg, fontFamily: "'Cairo', sans-serif", direction: "rtl" }}>
      ...
      <div style={{ margin: "14px 20px", background: C.card, borderRadius: 24, padding: 20, boxShadow: C.shadow }}>
        ...
        <button onClick={onBook} style={{ width: "100%", marginTop: 12, background: `linear-gradient(135deg, ${C.green}, ${C.greenDark})`, border: "none", borderRadius: 16, padding: "16px", color: "#fff", fontFamily: "inherit", fontWeight: 800, fontSize: 16, cursor: "pointer" }}>
          🚀 ابحث عن سيارة
        </button>
      </div>
    ...
    <div style={{ minHeight: "100vh", background: C.bg, fontFamily: "'Cairo', sans-serif", direction: "rtl", paddingBottom: 30 }}>
      ...
      <div style={{ margin: "14px 20px", background: C.card, borderRadius: 24, padding: 20, boxShadow: C.shadow }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 16 }}>
          {types.map(t => (
            <div key={t.id} onClick={() => setRideType(t.id)} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "14px 16px", borderRadius: 14, border: `2px solid ${rideType === t.id ? C.green : C.border}`, background: rideType === t.id ? C.greenLight : C.bg, cursor: "pointer" }}>
    ...
    <div style={{ minHeight: "100vh", background: C.bg, fontFamily: "'Cairo', sans-serif", direction: "rtl", paddingBottom: 40 }}>
      ...
      <div style={{ margin: "0 20px 14px", background: C.card, borderRadius: 18, padding: 6, display: "flex", boxShadow: C.shadow }}>
        {[{ id: "suggested", label: "✅ السعر المقترح" }, { id: "negotiate", label: "🤝 فاوض السعر" }].map(m => (
          <button key={m.id} onClick={() => setMode(m.id)} style={{ flex: 1, padding: "12px 8px", borderRadius: 14, border: "none", background: mode === m.id ? C.dark : "transparent", color: mode === m.id ? "#fff" : C.textMuted, cursor: "pointer", fontFamily: "inherit", fontWeight: 700, fontSize: 13 }}>
            {m.label}
          </button>
        ))}
      </div>
      {mode === "suggested" ? (
        ...
        <div style={{ margin: "0 20px" }}>
          <div style={{ background: C.card, borderRadius: 24, padding: 24, boxShadow: C.shadow, marginBottom: 14 }}>
            <div style={{ fontSize: 13, color: C.textMuted, marginBottom: 4 }}>سعرك {discount > 0 && `· وفّرت ${discount}%`}</div>
            ...
            {isLow && <div style={{ marginTop: 10, background: C.redLight, borderRadius: 10, padding: "8px 12px", fontSize: 12, color: C.red }}>⚠️ السعر منخفض جداً</div>}
          </div>
    ...
    return (
      <div style={{ minHeight: "100vh", background: C.bg, fontFamily: "'Cairo', sans-serif", direction: "rtl", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 24, gap: 16 }}>
        ...
        <div style={{ background: C.card, borderRadius: 24, padding: 24, width: "100%", boxShadow: C.shadow, textAlign: "center" }}>
          ...
          <button onClick={onEnd} style={{ width: "100%", background: `linear-gradient(135deg, ${C.green}, ${C.greenDark})`, border: "none", borderRadius: 14, padding: 16, color: "#fff", fontFamily: "inherit", fontWeight: 800, fontSize: 16, cursor: "pointer" }}>✅ إنهاء وتقييم</button>
        </div>
      </div>
    );
  }
      ...
      <div style={{ margin: "14px 20px", background: C.card, borderRadius: 24, padding: 20, boxShadow: C.shadow }}>
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 14 }}>
          ...
          <div style={{ textAlign: "center" }}>
            ...
            <div style={{ fontWeight: 700, color: C.text, fontSize: 13 }}>{booking.to}</div>
          </div>
...
export default function App() {
  ...
  return (
    <div style={{ maxWidth: 390, margin: "0 auto", minHeight: "100vh" }}>
      ...
      {authScreen === "welcome" && (
        <WelcomeScreen onSelectRole={role => {
      ...
      {authScreen === "form" && (
        <AuthForm
          role={selectedRole}
          onSuccess={() => setAuthScreen("app")}
          onBack={() => { setSelectedRole(null); setAuthScreen("welcome"); }}
        />
      )}

      {authScreen === "app" && !selectedRole && (
        <WelcomeScreen onSelectRole={role => setSelectedRole(role)} />
      )}

      {authScreen === "app" && selectedRole === "driver" && (
        <DriverDashboard onLogout={handleLogout} />
      )}

      {authScreen === "app" && selectedRole === "passenger" && (
        <PassengerApp onLogout={handleLogout} user={currentUser} />
      )}
    </div>
  );
}
...
function PassengerApp({ onLogout, user }) {
  ...
  return (
    <>
      {screen === SCREENS.HOME && <HomeScreen onBook={() => setScreen(SCREENS.BOOKING)} onLogout={onLogout} user={user} />}
