...
function ForceLogoutModal({ lang, onConfirm, onCancel }) {
  return (
    <div style={{ position:"fixed",inset:0,background:"rgba(0,0,0,0.65)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:3000,backdropFilter:"blur(4px)",padding:20 }}>
      <div style={{ background:C.card,borderRadius:24,padding:28,width:"100%",maxWidth:380,fontFamily:"Cairo,sans-serif",direction:lang==="ar"?"rtl":"ltr",textAlign:"center" }}>
        <div style={{ fontSize:56,marginBottom:12 }}>⚠️</div>
        <div style={{ fontWeight:900,fontSize:18,color:C.text,marginBottom:8 }}>
          {lang==="ar"?"تنبيه الجلسة":"Session Alert"}
        </div>
        <div style={{ fontSize:14,color:C.textMuted,marginBottom:20 }}>
          {lang==="ar"?"تم تسجيل الدخول إلى هذا الحساب من جهاز آخر. هل تريد متابعة الجلسة من هذا الجهاز وإلغاء الجهاز الآخر؟":
           "This account was accessed from another device. Do you want to continue on this device and invalidate the other?"}
        </div>
        <div style={{ display:"flex",gap:10 }}>
          ...
          <button onClick={onConfirm} style={{ flex:2,background:`linear-gradient(135deg,${C.green},${C.greenDark})`,border:"none",borderRadius:14,padding:14,color:"#fff",fontFamily:"inherit",fontWeight:800,cursor:"pointer" }}>
            {lang==="ar"?"المتابعة هنا":"Continue here"}
          </button>
        </div>
      </div>
    </div>
...
function ReportModal({ targetId, targetName, targetType, reporterId, reporterName, onClose, lang }) {
  const isRTL=lang==="ar";
  const reasons=REPORT_REASONS[lang]||REPORT_REASONS.ar;
...
// ===== AUTH =====
function AuthForm({ role, onSuccess, onBack, lang, setLang }) {
  const t = T[lang]||T.ar;
  const isRTL = lang === "ar";
  ...
  const saveName = async () => {
    if (!name.trim()) { setError(lang==="ar"?"أدخل اسمك":"Entrez votre nom"); return; }
    setLoading(true);
    try {
    ...
    <div style={{ minHeight:"100vh",background:C.bg,fontFamily:"Cairo,sans-serif",direction:isRTL?"rtl":"ltr" }}>
      ...
      <div style={{ background:`linear-gradient(135deg,${C.dark},#16213e)`,padding:"48px 24px 24px",textAlign:"center",position:"relative" }}>
        ...
        <div style={{ fontSize:12,color:"#ffffff66",marginTop:4 }}>
          {step==="phone"?(lang==="ar"?"أدخل رقم هاتفك":lang==="fr"?"Entrez votre numéro":"Enter your phone"):
           step==="otp"?(lang==="ar"?"أدخل رمز التحقق":"Code de vérification"):
           (lang==="ar"?"أدخل اسمك":"Votre prénom")}
        </div>
      </div>

      {/* Progress */}
      <div style={{ height:3,background:C.border }}>
        <div style={{ height:"100%",background:accent,width:step==="phone"?"33%":step==="otp"?"66%":"100%",transition:"width 0.4s ease" }} />
      </div>
      ...
      <div style={{ padding:"24px 20px 40px" }}>
        ...
        {step==="phone"&&(
          <div style={{ background:C.card,borderRadius:24,padding:24,boxShadow:C.shadow,display:"flex",flexDirection:"column",gap:14 }}>
            <div style={{ background:C.blueLight,borderRadius:12,padding:"10px 14px",fontSize:12,color:C.blue,textAlign:"center" }}>
              💡 {lang==="ar"?"المسجل يدخل برقمه مباشرة · الجديد يُنشئ حسابه تلقائياً":"Déjà inscrit: entrez votre numéro · Nouveau: compte créé automatiquement"}
            </div>
            ...
            <button onClick={sendOTP} disabled={loading||phone.replace(/\D/g,"").length<9}
              style={{ background:phone.replace(/\D/g,"").length>=9?`linear-gradient(135deg,${accent},${accentDark})`:C.border,border:"none",borderRadius:16,padding:16,color:"#fff",fontFamily:"inherit",fontWeight:800,fontSize:16,cursor:phone.replace(/\D/g,"").length>=9?"pointer":"default",opacity:loading?0.7:1,display:"flex",alignItems:"center",justifyContent:"center",gap:8 }}>
              {loading?<><span style={{ width:18,height:18,border:"2px solid #fff",borderTopColor:"transparent",borderRadius:"50%",display:"inline-block",animation:"spin 1s linear infinite" }} />{lang==="ar"?"جارٍ الإرسال...":"Envoi..."}</>:`📨 ${lang==="ar"?"إرسال رمز التحقق":lang==="fr"?"Envoyer le code":"Send code"}`}
            </button>
            <button onClick={()=>setShowReset(true)} style={{ background:"none",border:"none",color:C.blue,fontFamily:"inherit",fontSize:13,cursor:"pointer",fontWeight:600,textAlign:"center" }}>
              {t.forgotPass}
            </button>
          </div>
        )}
        ...
        {step==="name"&&(
          <div style={{ background:C.card,borderRadius:24,padding:24,boxShadow:C.shadow,display:"flex",flexDirection:"column",gap:12 }}>
            <div style={{ textAlign:"center",marginBottom:8 }}>
              ...
              <div style={{ fontWeight:800,fontSize:18,color:C.text }}>{lang==="ar"?"مرحباً بك! 🎉":"Bienvenue! 🎉"}</div>
              <div style={{ fontSize:13,color:C.textMuted,marginTop:4 }}>{lang==="ar"?"أدخل اسمك لإكمال التسجيل":"Entrez votre nom pour finaliser"}</div>
            </div>
            <input value={name} onChange={e=>setName(e.target.value)}
              placeholder={lang==="ar"?"اسمك الكامل":"Votre nom complet"} autoFocus
              style={{ background:C.bg,border:`1px solid ${C.border}`,borderRadius:14,padding:"16px",fontFamily:"inherit",fontSize:16,color:C.text,outline:"none",textAlign:isRTL?"right":"left" }} />
            <input value={referral} onChange={e=>setReferral(e.target.value.toUpperCase())}
              placeholder={t.referralCode}
              style={{ background:C.goldLight,border:`1px solid ${C.gold}44`,borderRadius:14,padding:"14px 16px",fontFamily:"inherit",fontSize:14,color:C.text,outline:"none",direction:"ltr",textAlign:"left" }} />
            {error&&<div style={{ background:C.redLight,borderRadius:12,padding:"10px 14px",fontSize:13,color:C.red,textAlign:"center" }}>{error}</div>}
            <button onClick={saveName} disabled={loading||!name.trim()}
              style={{ background:name.trim()?`linear-gradient(135deg,${C.green},${C.greenDark})`:C.border,border:"none",borderRadius:16,padding:16,color:"#fff",fontFamily:"inherit",fontWeight:800,fontSize:16,cursor:name.trim()?"pointer":"default",opacity:loading?0.7:1,display:"flex",alignItems:"center",justifyContent:"center",gap:8 }}>
              {loading?<><span style={{ width:18,height:18,border:"2px solid #fff",borderTopColor:"transparent",borderRadius:"50%",display:"inline-block",animation:"spin 1s linear infinite" }} />{lang==="ar"?"جارٍ...":"..."}</>:`🚀 ${lang==="ar"?"ابدأ!":"C'est parti!"}`}
            </button>
          </div>
        )}
      </div>
      ...
      <div style={{ minHeight:"100vh",background:C.bg,fontFamily:"Cairo,sans-serif",direction:isRTL?"rtl":"ltr" }}>
        ...
        <div style={{ margin:"14px 20px",background:C.card,borderRadius:24,padding:22,boxShadow:C.shadow }}>
          <div style={{ textAlign:"center",marginBottom:16 }}>
            ...
            <div style={{ fontWeight:900,fontSize:20,color:C.text }}>{t.tripOngoing}</div>
            <div style={{ fontSize:28,fontWeight:900,color:C.greenDark,fontFamily:"monospace",marginTop:4 }}>
              {String(mins).padStart(2,"0")}:{String(secs).padStart(2,"0")}
            </div>
          </div>
          <div style={{ background:C.bg,borderRadius:16,padding:16,marginBottom:16,display:"flex",flexDirection:"column",gap:10 }}>
            <div style={{ display:"flex",justifyContent:"space-between" }}>
              <span style={{ fontSize:13,color:C.textMuted }}>{t.dest}</span>
              <span style={{ fontSize:13,fontWeight:700,color:C.text,maxWidth:200,textAlign:"left" }}>{booking?.destText||"—"}</span>
            </div>
            <div style={{ display:"flex",justifyContent:"space-between" }}>
              <span style={{ fontSize:13,color:C.textMuted }}>{t.price}</span>
              <span style={{ fontSize:15,fontWeight:900,color:C.greenDark }}>{booking?.price} DA</span>
            </div>
          </div>
          <button onClick={()=>openNavigation(booking?.destLat||0,booking?.destLng||0)} style={{ width:"100%",background:C.blueLight,border:`1px solid ${C.blue}44`,borderRadius:14,padding:14,color:C.blue,fontFamily:"inherit",fontWeight:800,fontSize:14,cursor:"pointer",marginBottom:10 }}>
            {t.trackRoute}
          </button>
          <div style={{ display:"flex",gap:8,marginBottom:10 }}>
            <button onClick={()=>setShowChat(true)} style={{ flex:1,background:C.blueLight,border:`1px solid ${C.blue}44`,borderRadius:12,padding:12,color:C.blue,fontFamily:"inherit",fontWeight:700,cursor:"pointer",fontSize:13,display:"flex",alignItems:"center",justifyContent:"center",gap:6 }}>
              💬 {t.chat}
            </button>
            <button onClick={()=>setShowReport(true)} style={{ flex:1,background:`${C.orange}15`,border:`1px solid ${C.orange}44`,borderRadius:12,padding:12,color:C.orange,fontFamily:"inherit",fontWeight:700,cursor:"pointer",fontSize:13,display:"flex",alignItems:"center",justifyContent:"center",gap:6 }}>
              🚨 {lang==="ar"?"تبليغ":"Signaler"}
            </button>
          </div>
          <button onClick={finishRide} style={{ width:"100%",background:`linear-gradient(135deg,${C.green},${C.greenDark})`,border:"none",borderRadius:16,padding:16,color:"#fff",fontFamily:"inherit",fontWeight:800,fontSize:16,cursor:"pointer" }}>
            {t.arrived}
          </button>
        </div>
        <SOSButton passengerName={passengerName} bookingId={bookingId} />
...
export default function App() {
  ...
  const [role, setRole] = useState(localStorage.getItem("taxidz_role") || null);
  ...
  if (!isLoaded || initializing) {
    return (
      <div style={{ minHeight: "100vh", background: C.dark, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", fontFamily: "Cairo,sans-serif", color: "#fff" }}>
        <img src="/logo192.png" alt="AL-BURAQ" style={{ width: 80, height: 80, marginBottom: 16, animation: "logoPulse 2s ease-in-out infinite" }} onError={e=>e.target.style.display="none"} />
        <div style={{ fontSize: 16, fontWeight: 700 }}>جارٍ التحميل...</div>
        <style>{`@keyframes logoPulse{0%,100%{transform:scale(1)}50%{transform:scale(1.08)}}`}</style>
      </div>
    );
  }
  ...
  if (!user && !selectedRole) {
    return <WelcomeScreen onSelect={(r) => setSelectedRole(r)} lang={lang} setLang={setLang} />;
  }

  // 2. تسجيل الدخول
  if (!user && selectedRole) {
    return <AuthForm role={selectedRole} onSuccess={(r) => setRole(r)} onBack={() => setSelectedRole(null)} lang={lang} setLang={setLang} />;
  }
  ...
  // 4. لوحة تحكم السائق
  if (role === "driver") {
    return <DriverDashboard user={user} onLogout={handleLogout} lang={lang} setLang={setLang} />;
  }
