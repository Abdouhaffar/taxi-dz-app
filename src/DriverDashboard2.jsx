...
import { doc, onSnapshot, updateDoc, setDoc, getDoc, serverTimestamp, collection, query, where, addDoc, orderBy, getDocs, limit, increment } from "firebase/firestore";
...
//
// ملاحظة مهمة: المتصفحات تمنع تشغيل أي صوت عبر Web Audio API ما لم يُفعَّل عبر لمسة
// حقيقية من المستخدم أولاً — أي AudioContext يُنشأ داخل مستمع Firestore (وليس عبر ضغطة
// مباشرة) يبقى "معلّقاً" (suspended) ولا يصدر صوتاً أبداً رغم عدم وجود أي خطأ ظاهر.
// الحل: سياق صوتي واحد دائم يُفعَّل مرة واحدة عند أول تفاعل حقيقي (زر "تفعيل الاتصال")
// ثم يُعاد استخدامه لاحقاً من أي مكان دون الحاجة لتفاعل جديد.
...
const unlockAudio=()=>{
  try {
    ...
    // نغمة صامتة قصيرة جداً لإتمام "الفتح" الفعلي للسياق على بعض المتصفحات
    const g = sharedAudioCtx.createGain(); g.gain.value = 0;
    const o = sharedAudioCtx.createOscillator(); o.connect(g); g.connect(sharedAudioCtx.destination);
    o.start(); o.stop(sharedAudioCtx.currentTime+0.01);
  } catch(e){}
...
const alertNewBooking=()=>{
  ...
  try {
    if(!sharedAudioCtx) return; // لم يُفعَّل السياق الصوتي بعد (لم يضغط المستخدم على أي شيء بعد)
    if(sharedAudioCtx.state==="suspended") sharedAudioCtx.resume();
...
function DriverReportModal({ targetId, targetName, driverId, driverName, onClose }) {
  ...
  return (
    <div style={{ position:"fixed",inset:0,background:"rgba(0,0,0,0.7)",display:"flex",alignItems:"flex-end",justifyContent:"center",zIndex:2000,backdropFilter:"blur(4px)" }}>
      <div style={{ background:C.card,borderRadius:"24px 24px 0 0",padding:"28px 24px 40px",width:"100%",maxWidth:430,fontFamily:"'IBM Plex Sans Arabic',Cairo,sans-serif",direction:"rtl" }}>
        {done?(
...
function DriverPinChange({ uid, phone, currentPin, onClose }) {
  ...
  return (
    <div style={{ position:"fixed",inset:0,background:"rgba(0,0,0,0.7)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:2000,backdropFilter:"blur(4px)",padding:20 }}>
      <div style={{ background:C.card,borderRadius:24,padding:28,width:"100%",maxWidth:380,fontFamily:"'IBM Plex Sans Arabic',Cairo,sans-serif",direction:"rtl",border:`1px solid ${C.border}` }}>
        {done?(
...
function ChatBox({ bookingId, userId, userName, otherName, onClose }) {
  ...
  return (
    <div style={{ position:"fixed",inset:0,background:C.bg,zIndex:1500,display:"flex",flexDirection:"column",fontFamily:"'IBM Plex Sans Arabic',Cairo,sans-serif",direction:"rtl" }}>
...
function SubscriptionScreen({ uid, driverData, subscriptionEnabled, onBack }) {
  ...
  if (!subscriptionEnabled) return (
    <div style={{ minHeight:"100vh", background:C.bg, fontFamily:"'IBM Plex Sans Arabic',Cairo,sans-serif", direction:"rtl" }}>
  ...
  return (
    <div style={{ minHeight:"100vh", background:C.bg, fontFamily:"'IBM Plex Sans Arabic',Cairo,sans-serif", direction:"rtl", paddingBottom:40 }}>
      <div style={{ display:"flex", alignItems:"center", padding:"48px 20px 16px", gap:12 }}>
        <button onClick={onBack} style={{ width:40,height:40,borderRadius:12,background:C.card,border:`1px solid ${C.border}`,cursor:"pointer",fontSize:18,display:"flex",alignItems:"center",justifyContent:"center",color:C.text }}>←</button>
        <div style={{ fontWeight:800, fontSize:18, color:C.text }}>💳 الاشتراكات</div>
      </div>
...
function DriverVerificationForm({ uid, userEmail, existingData, onDone, isUpdate=false }) {
  ...
  const handleSubmit=async()=>{
    ...
    try {
      ...
      if(!isUpdate){
        const finalFirstName = existingData?.firstName || firstName;
        const finalLastName = existingData?.lastName || lastName;
        Object.assign(updateData,{ uid,email:userEmail||"",name:`${finalFirstName} ${finalLastName}`,firstName:finalFirstName,lastName:finalLastName,gender,birthDate,driverType,wilaya,daira,selfieUrl:selfieB64,carFrontUrl:carFrontB64,carSideUrl:carSideB64,licenseUrl:licenseB64||null,role:"driver",isOnline:false,location:null,rating:0,totalRatings:0,totalRides:0,points:0 });
      }
...
function PendingView({onLogout}) {
  return (
    <div style={{ minHeight:"100vh",background:C.bg,fontFamily:"'IBM Plex Sans Arabic',Cairo,sans-serif",direction:"rtl",display:"flex",alignItems:"center",justifyContent:"center",padding:24 }}>
...
function RatePassengerModal({ booking, onDone }) {
  ...
  return (
    <div style={{ position:"fixed",inset:0,background:"rgba(0,0,0,0.7)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:2000,backdropFilter:"blur(4px)",padding:20 }}>
      <div style={{ background:C.card,borderRadius:24,padding:28,width:"100%",maxWidth:380,fontFamily:"'IBM Plex Sans Arabic',Cairo,sans-serif",direction:"rtl",border:`1px solid ${C.border}`,textAlign:"center" }}>
...
function DriverNotificationToast({ notification, onClose }) {
  ...
  return (
    <div onClick={onClose} style={{ position:"fixed",top:12,left:12,right:12,zIndex:5000,background:C.card,border:`1px solid ${C.green}`,borderRadius:16,padding:"14px 16px",boxShadow:"0 8px 24px rgba(0,0,0,0.4)",cursor:"pointer",fontFamily:"'IBM Plex Sans Arabic',Cairo,sans-serif",direction:"rtl" }}>
...
export default function DriverDashboard({ user, onLogout }) {
  ...
  const [passengerPrefill,setPassengerPrefill]=useState(null);
  const [prefillLoading,setPrefillLoading]=useState(true);
  useEffect(()=>{
    if(status!=="none"||!user?.uid){ setPrefillLoading(false); return; }
    (async()=>{
      try {
        const pSnap = await getDoc(doc(db,"passengers",user.uid));
        if(pSnap.exists()){
          const pd = pSnap.data();
          const nameParts = (pd.name||"").trim().split(/\s+/);
          setPassengerPrefill({ gender:pd.gender||"", firstName:nameParts[0]||"", lastName:nameParts.slice(1).join(" ")||"", birthDate:pd.birthDate||"" });
        }
      } catch(e){}
  ...
  // شبكة أمان: تفعيل الصوت عند أول لمسة على الصفحة أياً كانت (لأن السائق غالباً
  // يصبح متصلاً تلقائياً دون الضغط على أي زر، فقد لا يمر أبداً عبر toggleOnline)
  useEffect(()=>{
  ...
  if(loading) return <div style={{ minHeight:"100vh",background:C.bg,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'IBM Plex Sans Arabic',Cairo,sans-serif" }}><div style={{ textAlign:"center" }}><img src="/logo192.png" alt="" style={{ width:80,height:80,objectFit:"contain",marginBottom:16 }} onError={e=>e.target.style.display="none"} /><div style={{ color:C.textMuted,fontSize:14 }}>جارٍ التحقق...</div></div></div>;
  if(status==="none"&&prefillLoading) return <div style={{ minHeight:"100vh",background:C.bg,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'IBM Plex Sans Arabic',Cairo,sans-serif" }}><div style={{ textAlign:"center" }}><img src="/logo192.png" alt="" style={{ width:80,height:80,objectFit:"contain",marginBottom:16 }} onError={e=>e.target.style.display="none"} /><div style={{ color:C.textMuted,fontSize:14 }}>جارٍ التحقق...</div></div></div>;
  if(status==="none") return <DriverVerificationForm uid={user?.uid} userEmail={user?.email} existingData={passengerPrefill} onDone={()=>{}} isUpdate={false} />;
    ...
    <div style={{ minHeight:"100vh",background:C.bg,fontFamily:"'IBM Plex Sans Arabic',Cairo,sans-serif",direction:"rtl" }}>
      ...
      {showSessionAlert&&(
        <div style={{ position:"fixed",inset:0,background:"rgba(0,0,0,0.9)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:9999,padding:24,backdropFilter:"blur(8px)" }}>
          <div style={{ background:C.card,borderRadius:24,padding:28,width:"100%",maxWidth:380,fontFamily:"'IBM Plex Sans Arabic',Cairo,sans-serif",direction:"rtl",border:`1px solid ${C.red}44` }}>
        ...
        {tab==="home"&&(<>
          ...
          {acceptedBooking&&driverScreen==="pickup"&&isLoaded&&(
            <div style={{ margin:"14px 0" }}>
              ...
              <div style={{ margin:"10px 20px",background:C.card,borderRadius:"6px 22px 22px 22px",padding:16,border:`2px solid ${C.orange}` }}>
          ...
          {acceptedBooking&&driverScreen==="ride"&&isLoaded&&(
            <div style={{ margin:"14px 0" }}>
              ...
              <div style={{ margin:"10px 20px",background:C.card,borderRadius:"6px 22px 22px 22px",padding:16,border:`2px solid ${C.green}` }}>
                <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12 }}>
          ...
          {online&&bookings.length>0&&!acceptedBooking&&(
            <div style={{ padding:"14px 20px 0" }}>
              ...
              {bookings.map(r=>(
                <div key={r.id} style={{ background:C.card,borderRadius:"6px 20px 20px 20px",padding:16,marginBottom:10,border:`1px solid ${C.orange}44` }}>
          ...
          {online&&bookings.length===0&&!acceptedBooking&&(
            <div style={{ margin:"16px 20px",background:C.card,borderRadius:"6px 24px 24px 24px",padding:32,border:`1px solid ${C.border}`,textAlign:"center" }}>
      ...
      <style>{`@import url('https://fonts.googleapis.com/css2?family=IBM+Plex+Sans+Arabic:wght@400;500;600;700&display=swap'); @keyframes pulse{0%,100%{opacity:1}50%{opacity:0.4}} @keyframes spin{to{transform:rotate(360deg)}}`}</style>
    </div>
  );
}
