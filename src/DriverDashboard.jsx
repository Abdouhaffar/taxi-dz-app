...
import { doc, onSnapshot, updateDoc, setDoc, serverTimestamp, collection, query, where, addDoc } from "firebase/firestore";
...
const REPORT_REASONS_PASSENGER = [
  "سلوك غير لائق", "إلغاء متكرر بعد القبول", "معلومات مزيفة",
  "تحرش أو إزعاج", "رفض الدفع", "تهديد أو عنف", "أخرى",
];
...
function DriverReportModal({ targetId, targetName, driverId, driverName, onClose }) {
  ...
  const handleSubmit = async () => {
    ...
    setSaving(true);
    try {
      await addDoc(collection(db,"reports"),{ targetId, targetName, targetType:"passenger", reporterId:driverId, reporterName:driverName, reason:selected==="أخرى"?custom:selected, status:"pending", createdAt:serverTimestamp() });
      setDone(true);
      ...
      <div style={{ background:C.card, borderRadius:"24px 24px 0 0", padding:"28px 24px 40px", width:"100%", maxWidth:430, fontFamily:"'Cairo',sans-serif", direction:"rtl" }}>
        {done ? (
          <div style={{ textAlign:"center", padding:"20px 0" }}>
            ...
            <div style={{ fontSize:13, color:C.textMuted, marginBottom:20 }}>سيتم مراجعته من قبل الإدارة</div>
          ...
          <>
            <div style={{ textAlign:"center", marginBottom:20 }}>
              ...
              <div style={{ fontWeight:900, fontSize:18, color:C.text }}>تبليغ عن الراكب</div>
              <div style={{ fontSize:13, color:C.textMuted }}>{targetName}</div>
            </div>
            <div style={{ display:"flex", flexDirection:"column", gap:8, marginBottom:16 }}>
              {REPORT_REASONS_PASSENGER.map((r,i)=>(
                <div key={i} onClick={()=>setSelected(r)} style={{ padding:"12px 16px", borderRadius:12, border:`2px solid ${selected===r?C.red:C.border}`, background:selected===r?C.redLight:C.bg, cursor:"pointer", fontSize:14, color:selected===r?C.red:C.text, fontWeight:selected===r?700:500, display:"flex", alignItems:"center", gap:10 }}>
                  <div style={{ width:18, height:18, borderRadius:"50%", border:`2px solid ${selected===r?C.red:C.textLight}`, background:selected===r?C.red:"transparent", flexShrink:0 }} />
                  {r}
                </div>
              ))}
            </div>
            {selected==="أخرى" && <textarea value={custom} onChange={e=>setCustom(e.target.value)} placeholder="اكتب سبب التبليغ..." rows={3} style={{ width:"100%", background:C.bg, border:`1px solid ${C.border}`, borderRadius:12, padding:"12px 16px", fontFamily:"inherit", fontSize:14, color:C.text, outline:"none", resize:"none", direction:"rtl", marginBottom:12 }} />}
            <div style={{ display:"flex", gap:10, marginTop:8 }}>
              ...
              <button onClick={handleSubmit} disabled={!selected||saving||(selected==="أخرى"&&!custom.trim())} style={{ flex:2, background:selected?`linear-gradient(135deg,${C.red},#dc2626)`:C.border, border:"none", borderRadius:14, padding:14, color:"#fff", fontFamily:"inherit", fontWeight:800, cursor:selected?"pointer":"default" }}>
      ...
      <div style={{ background:C.card, borderRadius:24, padding:28, width:"100%", maxWidth:380, fontFamily:"'Cairo',sans-serif", direction:"rtl", border:`1px solid ${C.border}` }}>
        {sent ? (
          <div style={{ textAlign:"center" }}>
            ...
            <div style={{ fontSize:13, color:C.textMuted, marginBottom:20 }}>تحقق من بريدك الإلكتروني واضغط على الرابط</div>
          ...
          <>
            <div style={{ textAlign:"center", marginBottom:20 }}>
              ...
              <div style={{ fontWeight:900, fontSize:18, color:C.text }}>نسيت كلمة المرور؟</div>
              <div style={{ fontSize:13, color:C.textMuted, marginTop:6 }}>أدخل بريدك وسنرسل لك رابط الاسترجاع</div>
            </div>
...
// ===== VERIFICATION FORM (مشترك للتسجيل وتغيير السيارة) =====
function DriverVerificationForm({ uid, userEmail, existingData, onDone, isUpdate=false }) {
  ...
  const validateStep = () => {
    ...
    if(step===2&&(!selfieB64||!carFrontB64||!carSideB64||!grayCardB64)&&!isUpdate){setError("يرجى رفع الصور الإلزامية ⚠️");return false;}
    ...
    try {
      ...
      if (!isUpdate) {
        Object.assign(updateData, {
          uid, email:userEmail||"", name:`${firstName} ${lastName}`,
          firstName, lastName, gender, birthDate, driverType, wilaya, daira,
          selfieUrl:selfieB64, carFrontUrl:carFrontB64, carSideUrl:carSideB64,
          licenseUrl:licenseB64||null, role:"driver", isOnline:false, location:null,
        });
      } else {
        if(selfieB64) updateData.selfieUrl = selfieB64;
        if(carFrontB64) updateData.carFrontUrl = carFrontB64;
        if(carSideB64) updateData.carSideUrl = carSideB64;
        if(licenseB64) updateData.licenseUrl = licenseB64;
      }
    ...
    <div style={{ minHeight:"100vh", background:C.bg, fontFamily:"'Cairo',sans-serif", direction:"rtl" }}>
      <div style={{ background:C.card, padding:"48px 20px 20px", borderBottom:`1px solid ${C.border}` }}>
        ...
        <div style={{ display:"flex", gap:6 }}>
          {stepsToShow.map((s,i)=><div key={s.id} style={{ flex:1, height:4, borderRadius:2, background:i<=currentStepIdx?C.orange:C.border, transition:"all 0.3s" }} />)}
        </div>
      </div>
      <div style={{ padding:"20px 20px 120px" }}>
        ...
        {step===2&&(<>
          {isUpdate&&<div style={{ background:`${C.blue}15`, borderRadius:12, padding:"10px 14px", marginBottom:16, border:`1px solid ${C.blue}44` }}><div style={{ fontSize:12, color:C.blue, fontWeight:600 }}>📌 البطاقة الرمادية إلزامية · باقي الصور اختيارية (تحديث)</div></div>}
        ...
        <div style={{ display:"flex", gap:10, marginTop:8 }}>
          ...
          {step<STEPS.length-1
            ...
            :<button onClick={handleSubmit} disabled={saving} style={{ flex:2, background:saving?C.border:`linear-gradient(135deg,${C.green},${C.greenDark})`, border:"none", borderRadius:14, padding:"14px", color:"#fff", fontFamily:"inherit", fontWeight:800, cursor:saving?"default":"pointer", fontSize:15, opacity:saving?0.7:1, display:"flex", alignItems:"center", justifyContent:"center", gap:8 }}>
              {saving?<><span style={{ width:16,height:16,border:"2px solid #fff",borderTopColor:"transparent",borderRadius:"50%",display:"inline-block",animation:"spin 1s linear infinite" }} />جارٍ الحفظ...</>:(isUpdate?"✅ إرسال للمراجعة":"✅ إرسال للمراجعة")}
            </button>}
        </div>
      </div>
  ...
  return (
    <div style={{ margin:"0 20px", borderRadius:16, overflow:"hidden", border:`2px solid ${mode==="pickup"?C.orange:C.green}` }}>
      ...
      <GoogleMap mapContainerStyle={{ width:"100%", height:"220px" }} center={driverLocation||passengerLocation||{lat:36.737,lng:3.086}} zoom={15} onLoad={m=>mapRef.current=m} options={{ disableDefaultUI:true, zoomControl:true }}>
        {driverLocation&&<Marker position={driverLocation} icon={{ url:makeMarker("🚕","#1a1a2e"), scaledSize:new window.google.maps.Size(44,44) }} />}
...
export default function DriverDashboard({user,onLogout}) {
  ...
  useEffect(()=>{
    ...
    const goOnline=async()=>{
      ...
      navigator.geolocation?.getCurrentPosition(async pos=>{
        ...
        if(window.OneSignal){window.OneSignal.getUserId?.(async pid=>{if(pid)await setDoc(doc(db,"drivers",user.uid),{oneSignalPlayerId:pid},{merge:true});});}
      });
    };
    goOnline();
  ...
  // طلبات pending فقط — الملغاة تختفي فوراً
  useEffect(()=>{
  ...
  const acceptBooking=async(booking)=>{
    try{
      await updateDoc(doc(db,"bookings",booking.id),{status:"accepted",driverId:user.uid,driverInfo:{name:data?.name||"السائق",phone:data?.phone||"",carBrand:data?.carBrand||"",carModel:data?.carModel||"",plateNumber:data?.plateNumber||"",rating:data?.rating||null,selfieUrl:data?.selfieUrl||null},acceptedAt:serverTimestamp()});
  ...
  if(loading) return <div style={{ minHeight:"100vh", background:C.bg, display:"flex", alignItems:"center", justifyContent:"center", fontFamily:"'Cairo',sans-serif" }}><div style={{ textAlign:"center" }}><div style={{ fontSize:48, marginBottom:16 }}>🔄</div><div style={{ color:C.textMuted }}>جارٍ التحقق...</div></div></div>;
  ...
  const stats=[
    {label:"أرباح اليوم",value:"—",icon:"💰",color:C.green},
    {label:"رحلات اليوم",value:data?.totalRides||0,icon:"🚕",color:C.blue},
    {label:"التقييم",value:data?.rating||"جديد ⭐",icon:"🏆",color:C.yellow},
    {label:"الطلبات",value:bookings.length,icon:"🔔",color:C.orange},
  ];
            ...
            <div style={{ margin:"14px 0" }}>
              ...
              <div style={{ margin:"10px 20px", background:C.card, borderRadius:18, padding:16, border:`2px solid ${C.green}` }}>
                ...
                <div style={{ display:"flex", gap:8, marginBottom:12 }}>
                  ...
                  <div style={{ flex:1, background:`${C.orange}22`, borderRadius:10, padding:"8px", textAlign:"center" }}><div style={{ fontSize:18, fontWeight:900, color:C.orange }}>🟢</div><div style={{ fontSize:10, color:C.textMuted }}>GPS</div></div>
                </div>
          ...
          <div style={{ padding:"20px 20px 100px" }}>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
              <button onClick={()=>setShowChangeVehicle(true)} style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:16, padding:"16px 20px", color:C.text, fontFamily:"inherit", fontWeight:700, cursor:"pointer", fontSize:14, display:"flex", alignItems:"center", gap:12, textAlign:"right" }}>
                ...
                <div style={{ flex:1 }}><div style={{ fontWeight:700 }}>تغيير السيارة</div><div style={{ fontSize:12, color:C.textMuted, marginTop:2 }}>تحديث بيانات سيارتك — يُعاد التحقق من الإدارة</div></div>
              ...
              <button onClick={()=>setShowReset(true)} style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:16, padding:"16px 20px", color:C.text, fontFamily:"inherit", fontWeight:700, cursor:"pointer", fontSize:14, display:"flex", alignItems:"center", gap:12, textAlign:"right" }}>
                <span style={{ fontSize:22 }}>🔐</span>
                <div style={{ flex:1 }}><div style={{ fontWeight:700 }}>تغيير / استرجاع كلمة المرور</div><div style={{ fontSize:12, color:C.textMuted, marginTop:2 }}>سنرسل رابط الاسترجاع لبريدك</div></div>
                <span style={{ color:C.textMuted }}>←</span>
              </button>
